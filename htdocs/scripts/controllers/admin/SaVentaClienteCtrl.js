...
sisAdminApp.controller('SaVentaClienteCtrl', 
              ...
              function ($scope,Restangular,$modal) {
                      ...
                      $scope.loadPage = function() {  
                            var listado = Restangular.all('ventacliente/list');
                            listado.getList().then(function(response){ $scope.listaMarcas = response.data;});
                        };

                      $scope.save = function() {
                          //$scope.ventacliente.clienteId=$scope.clienteSeleccionado.id;
                          if($scope.isEmpty($scope.ventacliente.id)) {
                                Restangular.all('ventacliente/add').post($scope.ventacliente).then(function(response) {
                                    $scope.openModalSave($scope.ventacliente);                                  
                      ...
                      $scope.delete = function(marca) {
                               ...
                               modalInstance.result.then(
                                  function (selectedItem) {
                                        Restangular.one('marca').remove({id:$scope.items[0].id}).then(
