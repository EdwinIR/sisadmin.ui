...
sisAdminApp.controller('SaRelacion979Ctrl', 
              ...
              function ($scope,Restangular,$modal) {
                        ...
                        var cuenta79 = Restangular.all('relacion79/list');
                          cuenta79.getList().then(function(response){
                              $scope.cuenta79=response.data;                                          
                        });

                        var cuentanueves = Restangular.all('relacion9/list');
                          cuentanueves.getList().then(function(response){
                              $scope.cuentanueves=response.data;                                          
                        });  
                       
                      $scope.loadPage = function() {  
                            var listado = Restangular.all('relacion979/list');
                            listado.getList().then(function(response){ $scope.listaRelacion979 = response.data;});
                        };

                      $scope.save = function() {
                        $scope.relacion979.cuentaDivisionaria9=$scope.cuentaNueveSeleccionada.id;
                        $scope.relacion979.cuentaContable79=$scope.cuenta79Seleccionada.id;
                          if($scope.isEmpty($scope.relacion979.id)) {
                                Restangular.all('relacion979/add').post($scope.relacion979).then(function(response) {
                                    $scope.openModalSave($scope.relacion979);                                  
                                    console.log(response.data);                    
                                    $scope.clear();
                                    $scope.loadPage();
                                });
                           } else {
                                Restangular.all('relacion979/update').post($scope.relacion979).then(function(response) {
                                    $scope.openModalSave($scope.relacion979);                                  
                                    console.log(response.data);                    
                                    $scope.clear();
                                    $scope.loadPage();
                      ...
                      $scope.delete = function(relacion979) {
                               $scope.items = [relacion979];
                               ...
                               modalInstance.result.then(
                                  function (selectedItem) {
                                        Restangular.one('relacion979').remove({id:$scope.items[0].id}).then(
                      ...
                      $scope.edit = function(id) {
                                      ...
                                      var i = 0;
                                      angular.forEach($scope.listaRelacion979, function(item) {
                                          if (item.id == id) {
                                                $scope.relacion979 = $scope.listaRelacion979[i];
                                          }
                                          i++;
                                      });
                      };
...
sisAdminApp.controller('MarcaPaginationCtrl', ['$scope',function ($scope) {
  $scope.$watch('listaRelacion979', function (newValue) {
    if (newValue) {
      $scope.totalItems = $scope.listaRelacion979.length;
      $scope.currentPage = 1;   
    }
  });
  $scope.currentPage = 1;
