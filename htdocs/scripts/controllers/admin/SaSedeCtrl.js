...
sisAdminApp.controller('SaSedeCtrl', 
              ...
              function ($scope,Restangular,$modal) {
                      ...
                      $scope.loadPage = function() {  
                            var listado = Restangular.all('sede/list');
                            listado.getList().then(function(response){ $scope.listaSedes = response.data;});
                        };

                      $scope.save = function() {
                          if($scope.isEmpty($scope.sede.id)) {
                                Restangular.all('sede/add').post($scope.sede).then(function(response) {
                                    $scope.openModalSave($scope.sede);                                  
                                    console.log(response.data);                    
                                    $scope.clear();
                                    $scope.loadPage();
                                });
                           } else {
                                Restangular.all('sede/update').post($scope.sede).then(function(response) {
                                    $scope.openModalSave($scope.sede);                                  
                                    console.log(response.data);                    
                                    $scope.clear();
                                    $scope.loadPage();
                      ...
                      $scope.delete = function(sede) {
                               ...
                               modalInstance.result.then(
                                  function (selectedItem) {
                                        Restangular.one('sede').remove({id:$scope.items[0].id}).then(
                      ...
                      $scope.edit = function(id) {
                                      ...
                                      var i = 0;
                                      angular.forEach($scope.listaSedes, function(item) {
                                          if (item.id == id) {
                                                $scope.sede = $scope.listaSedes[i];
                                          }
                                          i++;
                                      });
                      };
...
sisAdminApp.controller('MarcaPaginationCtrl', ['$scope',function ($scope) {
  $scope.$watch('listaSedes', function (newValue) {
    if (newValue) {
      $scope.totalItems = $scope.listaSedes.length;
      $scope.currentPage = 1;   
    }
  });
  $scope.currentPage = 1;
