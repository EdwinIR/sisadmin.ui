...
sisAdminApp.controller('SaSubCentroCostoCtrl', 
              ...
              function ($scope,Restangular,$modal) {
                        ...
                        var centrocostos = Restangular.all('centrocosto/list');
                          centrocostos.getList().then(function(response){
                              $scope.centrocostos=response.data;                                          
                        });
                       
                      $scope.loadPage = function() {  
                            var listado = Restangular.all('subcentrocosto/list');
                            listado.getList().then(function(response){ $scope.listaSubCentroCostos = response.data;});
                        };

                      $scope.save = function() {
                        $scope.subcentrocosto.centroCostoId=$scope.centroCostoSeleccionado.id;
                          if($scope.isEmpty($scope.subcentrocosto.id)) {
                                Restangular.all('subcentrocosto/add').post($scope.subcentrocosto).then(function(response) {
                                    $scope.openModalSave($scope.subcentrocosto);                                  
                                    console.log(response.data);                    
                                    $scope.clear();
                                    $scope.loadPage();
                                });
                           } else {
                                Restangular.all('subcentrocosto/update').post($scope.subcentrocosto).then(function(response) {
                                    $scope.openModalSave($scope.subcentrocosto);                                  
                                    console.log(response.data);                    
                                    $scope.clear();
                                    $scope.loadPage();
                      ...
                      $scope.delete = function(subcentrocosto) {
                               $scope.items = [subcentrocosto];
                               ...
                               modalInstance.result.then(
                                  function (selectedItem) {
                                        Restangular.one('subcentrocosto').remove({id:$scope.items[0].id}).then(
                      ...
                      $scope.edit = function(id) {
                                      ...
                                      var i = 0;
                                      angular.forEach($scope.listaSubCentroCostos, function(item) {
                                          if (item.id == id) {
                                                $scope.subcentrocosto = $scope.listaSubCentroCostos[i];
                                                $scope.centroCostoSeleccionado = $scope.getCentroCosto($scope.listaSubCentroCostos[i].descripcionCentroCosto);
                                          }
                                          i++;
                                      });
                      };

                      $scope.getCentroCosto=function(id) {
                                      var type = {};
                                      angular.forEach($scope.centrocostos, function(item) {
                      ...
                      $scope.openModalSave = function(user) {
                        ...
                        var customers = Restangular.all('customer/list');
                          ...
                          $scope.clear = function() {
                                  ...
                                  $scope.subcentrocosto={};
                                  $scope.centroCostoSeleccionado={};
...
sisAdminApp.controller('MarcaPaginationCtrl', ['$scope',function ($scope) {
  $scope.$watch('listaSubCentroCostos', function (newValue) {
    if (newValue) {
      $scope.totalItems = $scope.listaSubCentroCostos.length;
      $scope.currentPage = 1;   
    }
  });
  $scope.currentPage = 1;
