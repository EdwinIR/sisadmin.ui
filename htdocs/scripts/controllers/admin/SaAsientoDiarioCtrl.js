...
sisAdminApp.controller('SaAsientoDiarioCtrl', 
              ...
              function ($scope,Restangular,$modal) {
                      ...
                      $scope.loadPage = function() {  
                            var listado = Restangular.all('asientodiario/list');
                            listado.getList().then(function(response){ $scope.listaAsientosDiarios = response.data;});
                        };

                       $scope.save = function() {
                          if($scope.isEmpty($scope.asientodiario.id)) {
                                Restangular.all('asientodiario/add').post($scope.asientodiario).then(function(response) {
                                    $scope.openModalSave($scope.asientodiario);                                  
                                    console.log(response.data);                    
                                    $scope.clear();
                                    $scope.loadPage();
                                });
                           } else {
                                Restangular.all('asientodiario/update').post($scope.asientodiario).then(function(response) {
                                    $scope.openModalSave($scope.asientodiario);                                  
                                    console.log(response.data);                    
                                    $scope.clear();
                                    $scope.loadPage();
                      ...
                      $scope.delete = function(asientodiario) {
                               $scope.items = [asientodiario];
                               ...
                               modalInstance.result.then(
                                  function (selectedItem) {
                                        Restangular.one('asientodiario').remove({id:$scope.items[0].id}).then(
                      ...
                      $scope.edit = function(id) {
                                      ...
                                      var i = 0;
                                      angular.forEach($scope.listaAsientosDiarios, function(item) {
                                          if (item.id == id) {
                                                $scope.asientodiario = $scope.listaAsientosDiarios[i];
                                          }
                                          i++;
                                      });
                      };
...
sisAdminApp.controller('MarcaPaginationCtrl', ['$scope',function ($scope) {
  $scope.$watch('listaAsientosDiarios', function (newValue) {
    if (newValue) {
      $scope.totalItems = $scope.listaAsientosDiarios.length;
      $scope.currentPage = 1;   
    }
  });
  $scope.currentPage = 1;
