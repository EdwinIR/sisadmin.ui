...
sisAdminApp.controller('SaKardexXProductoCtrl', ['$scope', 'Restangular', '$modal',
    function($scope, Restangular, $modal) {
        ...
        var applyFilter = function (filterData) {
                Restangular.one('kardexxproducto').post('filter',filterData).then(function(response) {
                        $scope.listaKardexProducto = response.data;
...
sisAdminApp.controller('MarcaPaginationCtrl', ['$scope', function($scope) {
    $scope.$watch('listaKardexProducto', function(newValue) {
        if (newValue) {
            $scope.totalItems = $scope.listaKardexProducto.length;
            $scope.currentPage = 1;
        }
    });
    $scope.currentPage = 1;
