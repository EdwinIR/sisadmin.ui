...
sisAdminApp.controller('SaListaXPrecioCtrl', ['$scope', 'Restangular', '$modal', 'LoginFactory',
    function($scope, Restangular, $modal ,LoginFactory) {
        ...
        var applyFilter = function (filterData) {
                $scope.pbValue=50;
                Restangular.one('listaprecio').post('filter',filterData).then(function(response) {
                        $scope.listaPrecio = response.data;
        ...
        $scope.bajarPdf = function(){
              ...
              Restangular.one('listaprecio/pdf').
              ...
              var fname = 'Lista_por_Precio'+$scope.fechaActual+'.PDF';
              saveAs(blob, fname);
        });
      };
